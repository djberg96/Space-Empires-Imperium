...
\textbf{Exception:} Treat a defending PD unit as a troop counter for purposes of this rule. It still only defends, but its presence protects the other non-troop counters from elimination. However, during placement it must be selected before other non-troop counters (if a non-troop counter is selected at all).

\textit{Rationale:} The rules as written state that there is only ever one round of combat per Surface sub-phase, stating "A planetary surface box cannot be conquered in one sub-phase if the number of attacking troops is less than the number of defending counters". However, this quickly leads to nonsensical tactics where the players can simply land hordes of cheap units (transports, fighters, scouts, etc) on the surface that make conquest of any planet virtually impossible given the counter mix and time constraints. This makes more thematic sense and also makes troops (and to a lesser extent PD units) more valuable.
