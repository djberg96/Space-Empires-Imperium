...
package on the Fleet Group Manifest of the separate Logistics \& Fleet Record
aid. All ships in a group must have the same package. Different groups of the
same ship class may select different packages.
...
The separate landscape Battle Board may be used as this display. Assign each
fleet a side, record the system, attacker, round, and range, then place every
revealed group in its printed Class row. Move groups between the Unscreened and
Screened columns as screening changes each round; keep numeral, damage,
package, and status markers with their group.
